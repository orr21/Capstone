...
def get_openai_client():
    ...
    host = os.environ.get("DATABRICKS_HOST", "https://dbc-6805aac2-7b4a.cloud.databricks.com").rstrip("/")
...
def generate_paper_quiz(paper_id: str) -> dict:
    """
    Generates a 3-question comprehension quiz dynamically generated by LLM for a research paper in Lakebase.
    ...
    try:
        ...
        # Dynamically generate quiz via LLM
        try:
            client = get_openai_client()
            prompt = (
                f"You are an academic quiz creator. Generate a 3-question multiple choice comprehension quiz for the paper:\n"
                f"Title: {title}\n"
                ...
                f'      "options": ["Correct Option", "Wrong 1", "Wrong 2", "Wrong 3"],\n'
                f'      "correct_option": 0\n'
                f'    }}\n'
                f'  ]\n'
                f"}}\n"
            ...
            response = client.chat.completions.create(
                model=os.environ.get("ENDPOINT_NAME", "databricks-meta-llama-3-3-70b-instruct"),
            ...
            logger.warning("LLM quiz generation fallback: %s", llm_err)
            abstract_snippet = abstract_text[:200]
            questions = [
                {
                    "id": 1,
                    "question": f"What is the primary focus of '{title[:50]}'?",
                    "options": [f"Research regarding {abstract_snippet[:60]}...", "Manual legacy processes", "Historical overview", "Database indexing"],
                    "correct_option": 0
                },
                {
                    "id": 2,
                    "question": "What is a key methodology or finding described in the abstract?",
                    "options": [f"Findings related to {abstract_snippet[60:120]}...", "Unrelated industrial process", "Purely theoretical baseline", "Standard catalog search"],
                    "correct_option": 0
                },
                {
                    "id": 3,
                    "question": "Why is this research paper relevant to your study goal?",
                    "options": ["Provides domain evidence and methodology", "Has no research value", "Deprecated methodology", "Random article"],
                    "correct_option": 0
                }
            ]
